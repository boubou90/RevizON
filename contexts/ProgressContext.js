...
export const ProgressProvider = ({ children }) => {
  ...
  const markQuizComplete = (subject, lessonId) => {
    const key = `${subject}_${lessonId}`;
    if (!progress.completedQuizzes.includes(key)) {
      const newProgress = {
        ...progress,
        completedQuizzes: [...progress.completedQuizzes, key]
      };
      saveProgress(newProgress);
    }
  };

  const isQuizCompleted = (subject, lessonId) => {
    return progress.completedQuizzes.includes(`${subject}_${lessonId}`);
  };
